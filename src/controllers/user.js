...
const verifyUser = async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(400).send({ Message: "<h2>Invalid user link</h2>" });
    ...
    if (!token)
      return res.status(400).send({ Message: "<h2>Invalid token</h2>" });
    ...
    res
      .status(400)
      .send({ Message: " An error occured verifying your email " });
  }
};
...
const login = async (req, res, next) => {
  ...
  try {
    if (!user.isVerified) {
      return res
        .status(400)
        .send({ Message: "You need to verify your account before signing in" });
    }
